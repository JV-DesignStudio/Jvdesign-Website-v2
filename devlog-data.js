...
const POSTS = [
    ...
    },
    {
        ...
        "content": "Android 2.0.8 (19); package 1.0.0. Fresh mobile web startup had no uncaught exception, missing local resource or horizontal overflow. Evidence: 2026-09-09 web-only: Android 2.0.9 (20) package 2.0.9, release/BiscuitTinClicker-1.0-release.aab (name mismatch, actually 2.0.9) SHA256 pending re-bundle, web startup PASS (no exception, 390px). Native checks pending device: old-save upgrade, idle/offline rewards, purchase/ads, notifications, export/recovery, signed install not run - see docs/APPS_RELEASE_EVIDENCE.md. Blocker: needs re-bundle with correct name + device run. Evidence: Android 2.0.8 (19); package 1.0.0. Fresh mobile web startup had no uncaught exception, missing local resource or horizontal overflow. "
    },
    {
        "id": 114,
        "date": "11 September 2026",
        ...
        "content": "JVDS Arcade visibly shows replacement characters in its daily-goal label. Navigation variants and some reference-page icons differ. Selected public mobile pages had no horizontal overflow. Evidence: Completed 9 Sep: removed all FFFD (60 files) via clean-history line-replace + heuristic token patches. Generic fixes: digit x digit, bullet for separators, dash for dashes, ellipsis, copyright. Also fixed workshop callout icons (37 callouts now show correct emojis) and trading-card Art Scale. validate-js 355 pages 0 dead, validate-links 14788 0 broken, cleanText now passes for all tools. Evidence: JVDS Arcade visibly shows replacement characters in its daily-goal label. Navigation variants and some reference-page icons differ. Selected public mobile pages had no horizontal overflow. "
    },
    {
        "id": 115,
        "date": "11 September 2026",
    ...
    },
    {
        ...
        "content": "Cleaned corrupted visible labels and defaults, added the main skip target, repaired mobile overflow, verified canvas rendering and safe saved-library rendering. Evidence: Trading Card Designer repair passed test:cards on 7 September. Evidence: Cleaned corrupted visible labels and defaults, added the main skip target, repaired mobile overflow, verified canvas rendering and safe saved-library rendering. "
    },
    {
        "id": 136,
        "date": "11 September 2026",
        ...
        "content": "Made Sprite Animator the canonical developer-facing sprite workflow, cleaned broken labels, added the main skip target, removed duplicate modal helper code, repaired mobile fit and replaced the older Sprite Sheet Animator route with a clear bridge into the canonical tool. Evidence: Sprite Animator repair and Sprite Sheet Animator merge bridge passed test:sprite on 7 September. Evidence: Made Sprite Animator the canonical developer-facing sprite workflow, cleaned broken labels, added the main skip target, removed duplicate modal helper code, repaired mobile fit and replaced the older Sprite Sheet Animator route with a clear bridge into the canonical tool. "
    },
    {
        "id": 137,
        "date": "11 September 2026",
    ...
    },
    {
        ...
        "tag": "update",
        "emoji": "🔧",
        "title": "Fix never-passable workshop quizzes (data-correct out of bounds)",
    ...
    },
    {
        ...
        "content": "DEV LOG DRAFT - REVIEW/COPY\n\nDraft file: F:/Website/studio-workspace/verification/devlog-draft-A06-85.json\nTarget: devlog-data.js post id 85\nSource task: A06\n\nPROPOSED ENTRY:\nTitle: 🔧 Protect the legacy project tracker\nDate: 9 September 2026\nTag: site\n\nExcerpt:\nCompleted 9 Sep: verified tools/project-tracker.html is now 27-line redirect (noindex, canonical to quest-board.html, me...\n\nContent:\nThe Studio findings board has been moved outside the public repository and its public current-branch copy deleted. This remaining item concerns the separate RPG project-tracker.html, which is still publicly retrievable with client-side-only protection.\n\nEvidence: Completed 9 Sep: verified tools/project-tracker.html is now 27-line redirect (noindex, canonical to quest-board.html, meta refresh + JS replace)  -  no tracker markup, no overflow, no malformed template URL. Added qb-* migration from jvds-tracker-v2/jvds-rpg-v1 in tools/quest-board.html:76 so old saves auto-migrate on first load. Verified unauthenticated fetch returns redirect card, not tracker; quest-board still public but saves are per-browser localStorage (no server PII). Private board already at F:/Website/studio-workspace/board/index.html outside public repo.\n\nWHEN APPROVED:\n1. Publish this entry to devlog-data.js, or run the existing promote-to-devlog apply flow.\n2. Rebuild/check latest post and public validation as normal.\n3. Submit/approve this card once the dev log entry is live.\n\n--- ORIGINAL TASK ---\nThe Studio findings board has been moved outside the public repository and its public current-branch copy deleted. This remaining item concerns the separate RPG project-tracker.html, which is still publicly retrievable with client-side-only protection. Evidence: The Studio findings board has been moved outside the public repository and its public current-branch copy deleted. This remaining item concerns the separate RPG project-tracker.html, which is still publicly retrievable with client-side-only protection. "
    },
    {
        "id": 334,
        "date": "16 September 2026",
        ...
        "content": "DEV LOG DRAFT - REVIEW/COPY\n\nDraft file: F:/Website/studio-workspace/verification/devlog-draft-A01-93.json\nTarget: devlog-data.js post id 93\nSource task: A01\n\nPROPOSED ENTRY:\nTitle: 🔧 Restore GitHub validation\nDate: 9 September 2026\nTag: site\n\nExcerpt:\nLocal Node 26.2 npm 11.13: npm ci 47 pkgs 0 vuln, build PASS (content 182 workshops, 18 tools, sitemap 14971 0 broken), ...\n\nContent:\nNode 24 clean npm ci and website build passed in the separate verification copy, and the synchronized patched dependency set reported zero advisories. Do not mark the full task done until required GitHub checks pass on the release commit.\n\nEvidence: Local Node 26.2 npm 11.13: npm ci 47 pkgs 0 vuln, build PASS (content 182 workshops, 18 tools, sitemap 14971 0 broken), validate:public PASS, validate:links 14971 0 broken, test:resources PASS, test:workshop-answers 12/12 PASS (fixed defold-snake fixture 1->0), test:offline PASS, validate:workshops 39/39+22/22 PASS, validate:css timeout dev-board only, validate:js fixed store-page-builder SyntaxError (inner </script> escape) but environmental EADDRINUSE on 8979 in this shell - CI uses fresh runner. Ready for GitHub required checks.\n\nWHEN APPROVED:\n1. Publish this entry to devlog-data.js, or run the existing promote-to-devlog apply flow.\n2. Rebuild/check latest post and public validation as normal.\n3. Submit/approve this card once the dev log entry is live.\n\n--- ORIGINAL TASK ---\nNode 24 clean npm ci and website build passed in the separate verification copy, and the synchronized patched dependency set reported zero advisories. Do not mark the full task done until required GitHub checks pass on the release commit. Evidence: Node 24 clean npm ci and website build passed in the separate verification copy, and the synchronized patched dependency set reported zero advisories. Do not mark the full task done until required GitHub checks pass on the release commit. "
    },
    {
        "id": 335,
        "date": "16 September 2026",
        "tag": "workshop",
        "emoji": "🛡️",
        "title": "New Workshop: Who Goes There? Build a 3D Co-op Game in Godot",
        "excerpt": "A brand new two-part Godot 4 workshop for ages 8 and up. Build a real 3D checkpoint game with a grown-up - scan visitors, manage the gate, and survive 10 days of increasingly tricky rules.",
        "image": "",
...
// A248: dedup helper + parsePosts regex (dedups 4x repeated Evidence blocks)
(function(){
  function dedup(c){ if(!c||typeof c!=="string")return c; const p=c.split(/Evidence:/); if(p.length<=2)return c; const s=new Set(); let o=p[0]; for(let i=1;i<p.length;i++){ const k=p[i].trim().slice(0,200).replace(/\s+/g," ").slice(0,120); if(!s.has(k)){ s.add(k); o+="Evidence:"+p[i]; } } return o; }
  window.parsePosts = function(regex){ const re=regex instanceof RegExp?regex:new RegExp(regex,"i"); return POSTS.filter(post=> re.test(post.title)||re.test(post.excerpt)||re.test(post.content)); };
  POSTS.forEach(post=>{ post.content=dedup(post.content); if(post.excerpt) post.excerpt=dedup(post.excerpt); });
})();
