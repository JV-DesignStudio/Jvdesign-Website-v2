...
(async () => {
  ...
  await page.evaluate(() => toggleLock(0)); // lock swatch 0
  const lockedBefore = await page.$eval('.swatch-hex', el => el.textContent);
  const lockedClass = await page.evaluate(() => palette[0].locked);
  await page.evaluate(() => shuffleUnlocked());
  ...
  // Full shuffle: retry up to 3 times to avoid probabilistic hex collision
  let fullShuffleOk = false;
  for (let attempt = 0; attempt < 3; attempt++) {
    await page.evaluate(() => shufflePalette());
    const afterFullShuffle = await page.$eval('.swatch-hex', el => el.textContent);
    if (afterFullShuffle !== lockedBefore) { fullShuffleOk = true; break; }
  }
  check('full shuffle reshuffles even locked colour', fullShuffleOk);

  // Keyboard: Enter on focused swatch toggles lock back ON
  await page.focus('.swatch');
  await page.keyboard.press('Enter');
  const kbLocked = await page.evaluate(() => palette[0].locked);
  ...
  await page.evaluate(() => savePalette());
  ...
  await page.evaluate(() => document.querySelector('.saved-item').click());
