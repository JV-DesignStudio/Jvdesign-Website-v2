...
const IGNORE_DIRS = new Set(['node_modules', '.git', '.claude', 'partials', 'quest-board-deploy', '.github', '.continue']);
...
for (const filePath of walk(ROOT)) {
  ...
  for (const m of c.matchAll(/(?:href|src)="([^"]+)"/g)) {
    ...
    refCount++;
    const target = clean.startsWith('/') ? path.join(ROOT, clean) : path.resolve(dir, clean);
