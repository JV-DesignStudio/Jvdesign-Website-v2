...
const port = 8986;
const pagePath = '/tools/gdd-builder.html';
...
(async () => {
  ...
  try {
    for (const vp of viewports) {
      console.log(`Checking GDD Builder at ${vp.name}...`);
      ...
      try {
        ...
        const result = await page.evaluate(() => {
          ...
          document.querySelectorAll('body *').forEach(el => {
            ...
            if (el.closest('.hero-btns, .controls-bar')) return;
          ...
          return {
            ...
            hasProgress: Boolean(document.getElementById('progressFill')),
            hasSections: document.querySelectorAll('.gdd-section').length,
        ...
        if (!result.hasProgress || result.hasSections < 9) failures.push(`${vp.name}: missing progress or document sections`);
  ...
  if (failures.length) {
    console.error('GDD Builder smoke failed:');
    failures.forEach(f => console.error('- ' + f));
    process.exit(1);
  }

  console.log(`GDD Builder smoke passed across ${viewports.length} viewports.`);
})();
