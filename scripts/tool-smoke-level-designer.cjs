...
const port = 8988;
const pagePath = '/tools/level-designer.html';
...
(async () => {
  ...
  try {
    for (const vp of viewports) {
      console.log(`Checking Level Designer at ${vp.name}...`);
      ...
      try {
        ...
        const result = await page.evaluate(() => {
          ...
          document.querySelectorAll('body *').forEach(el => {
            ...
            if (el.closest('.brief-steps, .level-actions, #bottom-bar, #pixel-toolbar, .tile-palette, .zoom-row, #sel-toolbar')) return;
          ...
          return {
            ...
            hasBrief: Boolean(document.querySelector('.level-brief')),
            stepCount: document.querySelectorAll('.brief-steps li').length,
            hasCanvas: Boolean(document.getElementById('level-canvas')),
            hasPanels: Boolean(document.getElementById('left-panel') && document.getElementById('right-panel')),
            hasPlayOverlay: Boolean(document.getElementById('play-overlay')),
        ...
        if (!result.hasBrief || result.stepCount !== 4) failures.push(`${vp.name}: missing level workflow brief`);
        if (!result.hasCanvas || !result.hasPanels || !result.hasPlayOverlay) failures.push(`${vp.name}: missing a core level designer area`);
  ...
  if (failures.length) {
    console.error('Level Designer smoke failed:');
    failures.forEach(f => console.error('- ' + f));
    process.exit(1);
  }

  console.log(`Level Designer smoke passed across ${viewports.length} viewports.`);
})();
